<div class="container-fluid">
    ...
    <div class="card shadow-sm mb-4 border-0 bg-light">
        <div class="card-body py-3">
            <form id="formFiltros" class="row g-2 align-items-center">
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        ...
                        <input type="text" id="filtroKeyword" class="form-control form-control-sm" placeholder="Buscar por reunión, tema u objetivo...">
                    </div>
                </div>

                <div class="col-md-1 text-end">
                    <button type="button" id="btnLimpiar" class="btn btn-sm btn-outline-secondary w-100" title="Limpiar Filtros">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="tablaMinutas">
                    <thead class="table-light">
                        <tr>
                            <th width="5%">ID</th>
                            <th width="20%">Nombre Reunión</th>
                            <th width="20%">Comisión</th>
                            <th width="25%">Temas Tratados</th>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderTabla(datos) {
            ...
            if (!datos || datos.length === 0) {
                tbody.innerHTML = '<tr><td colspan="7" class="text-center py-4 text-muted">No se encontraron resultados.</td></tr>';
                return;
            }

            datos.forEach(m => {
                let temasHtml = '<span class="text-muted small fst-italic">Sin temas registrados</span>';
                if (m.listaTemas) {
                    // NOTA: '|||' es el separador que usamos en el PHP (GROUP_CONCAT)
                    const temas = m.listaTemas.split('|||');
                    temasHtml = temas.map(t => `<div class="text-truncate" style="max-width: 300px;" title="${t}">• ${t}</div>`).join('');
                }
                ...
                let btnAdj = `<button class="btn btn-sm btn-light disabled border-0 text-muted"><i class="fas fa-paperclip"></i></button>`;
                if (numAdj > 0) {
                    btnAdj = `<button class="btn btn-sm btn-primary position-relative" onclick="verAdjuntos(${m.idMinuta})">
        ...
        fetch(`index.php?action=api_ver_adjuntos_minuta&id=${idMinuta}`)
            ...
            .then(resp => {
                ...
                if (resp.data && resp.data.length > 0) {
                    ...
                    resp.data.forEach(a => {
                        // Nombre a mostrar
                        let nombreMostrar = a.nombreArchivo ? a.nombreArchivo : a.pathAdjunto.split('/').pop();

                        // 1. Configuración por defecto (archivos locales)
                        let urlVisor = `index.php?action=ver_archivo_adjunto&id=${a.idAdjunto}`;
                        let extension = a.pathAdjunto.split('.').pop().toLowerCase();
                        ...
                        else if (['jpg', 'jpeg', 'png'].includes(extension)) icon = '<i class="fas fa-file-image text-info"></i>';

                        // 3. Lógica para Links Externos y Corrección de "www"
                        // Detectamos si es tipo link, o empieza por http, o empieza por www
