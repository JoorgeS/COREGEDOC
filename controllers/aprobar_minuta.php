...
// -----------------------------------------------------------------------------
// Ruta raíz del proyecto (subiendo dos niveles desde controllers)
define('ROOT_PATH', dirname(__DIR__) . '/'); // Asegura que no termine con '/' innecesaria
...
// 3. FUNCIÓN PARA GENERAR HTML (Adaptada de tu exportar_pdf.php)
// -----------------------------------------------------------------------------
function generateMinutaHtml($data, $logo_uri)
{
    // Generar cadena de comisiones
    $comisiones = htmlspecialchars($data['nombreComision1'] ?? 'N/A'); // Usar nombres
    if (!empty($data['nombreComision2'])) {
        $comisiones .= ' / ' . htmlspecialchars($data['nombreComision2']);
    }

    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Minuta Aprobada CORE</title><style>' .
        'body{font-family:Helvetica,sans-serif;font-size:10pt;line-height:1.5;}' .
        '.container{width:90%;margin:20px auto;}' .
        '.header-box{border-bottom:1px solid #ccc;padding-bottom:15px;margin-bottom:15px;overflow:hidden;}' .
        '.logo{width:60px;height:auto;float:left;margin-right:15px;}' .
        '.header-text{float:left;width:calc(100% - 80px);}' . /* Ajuste para logo */
        ...
        '.minuta-info th{background-color:#eee;width:25%;font-weight:bold;}' . /* Ajuste ancho */
        'h2{font-size:14pt;margin:25px 0 10px 0;color:#004d40;border-bottom:1px solid #ccc;padding-bottom:5px;}' .
        'h3{font-size:11pt;margin:15px 0 5px 0;color:#333;font-weight:bold;}' .
        '.content p{margin:5px 0 10px 0;}' .
        '.asistencia-list ul{list-style-type:disc;padding-left:20px;margin:10px 0;columns:2;}' . /* 2 columnas */
        ...
        '<div class="header-box"><img src="' . htmlspecialchars($logo_uri) . '" class="logo" alt="Logo"><div class="header-text"><p>GOBIERNO REGIONAL. REGIÓN DE VALPARAÍSO</p><p class="main-title">CONSEJO REGIONAL</p><p>COMISIÓN ' . strtoupper($comisiones) . '</p></div></div>' .
        '<table class="minuta-info">' .
        '<tr><th>MINUTA DE REUNIÓN</th><td colspan="3">APROBADA</td></tr>' .
        '<tr><th>Fecha</th><td>' . htmlspecialchars($data['fechaMinuta'] ?? 'N/A') . '</td><th>Hora</th><td>' . htmlspecialchars($data['horaMinuta'] ?? 'N/A') . '</td></tr>' .
        '<tr><th>Presidente</th><td>' . htmlspecialchars($data['nombrePresidente1'] ?? 'N/A') . '</td><th>Secretario Técnico</th><td>' . htmlspecialchars($data['nombreSecretario'] ?? 'N/A') . '</td></tr>' .
        '<tr><th>N° Sesión</th><td>' . htmlspecialchars($data['numeroSesion'] ?? 'N/A') . '</td><th>Lugar</th><td>Salón Plenarios</td></tr>' .
        (!empty($data['nombreComision2']) ? '<tr><th>Comisión Mixta</th><td>' . htmlspecialchars($data['nombreComision2']) . '</td><th>Presidente Mixta</th><td>' . htmlspecialchars($data['nombrePresidente2']) . '</td></tr>' : '') .
        '</table>' .
        '<h2>ASISTENTES</h2><div class="asistencia-list"><ul>';
    ...
    if (!empty($data['temas']) && is_array($data['temas'])) {
        foreach ($data['temas'] as $index => $tema) {
            if (empty(strip_tags($tema['nombreTema'] ?? ''))) continue; // Saltar temas vacíos
            $num = $index + 1;
            $html .= '<div class="tema-block"><h3>TEMA ' . $num . ': ' . strip_tags($tema['nombreTema']) . '</h3><div class="content">';
    ...
    // Acuerdos / Varios (Placeholder si no los tienes)
    $html .= '<h2>ACUERDOS GENERALES</h2><p>N/A</p>';
    $html .= '<h2>VARIOS</h2><p>N/A</p>';

    // Firma
    $html .= '<div class="signature-box"><div class="signature-line"></div><p>' . htmlspecialchars($data['nombrePresidente1'] ?? 'N/A') . '</p><p>Presidente</p><p>Comisión ' . $comisiones . '</p></div>';
    $html .= '</div></body></html>';
...
// -----------------------------------------------------------------------------
$db = null; // Para asegurar que se inicializa
$pdfWebPath = null; // Ruta web del PDF generado
$idDocumentoCreado = null; // ID del registro en t_documento
...
try {
    ...
    // Necesitamos una consulta más completa o varias consultas
    $datosParaPDF = [];

    // Datos básicos de la minuta
    $sql_minuta_data = "SELECT m.fechaMinuta, m.horaMinuta, m.t_comision_idComision, m.t_usuario_idPresidente
                        FROM t_minuta m
    ...
    // Nombres (Comisión 1, Presidente 1, Secretario) - Asumiendo que secretario es el logueado
    $datosParaPDF['nombreSecretario'] = $nombrePresidenteLogueado; // Usar el nombre del logueado como secretario? Ajusta según tu lógica.
    // Necesitas buscar los nombres basados en los IDs
    // Comisión 1
    ...
    // (Añadir lógica similar para Comisión 2 y Presidente 2 si usas comisión mixta)
    ...
    // (Añadir numeroSesion si lo tienes en alguna tabla o sesión)
    ...
    $sql_update_status = "UPDATE t_minuta
                          SET estadoMinuta = 'APROBADA',
                              fechaAprobacion = NOW()
                          WHERE idMinuta = :idMinuta AND estadoMinuta = 'PENDIENTE'"; // Seguridad doble
    ...
    $options->set('chroot', ROOT_PATH); // Directorio raíz para recursos locales (logo)
    $options->set('isPhpEnabled', true);

    $dompdf = new Dompdf($options);

    // Ruta relativa del logo desde la raíz del proyecto
    $logo_relative_path = 'public/img/logo2.png'; // Ajusta si tu logo está en otro sitio
    ...
    $saveDir = ROOT_PATH . 'public/docs/minutas_aprobadas/'; // Directorio de guardado (¡DEBE EXISTIR Y SER ESCRIBIBLE!)
    $filename = "minuta_aprobada_" . $idMinuta . "_" . date('Ymd_His') . ".pdf";
    $fullSavePath = $saveDir . $filename;
    $pdfWebPath = "/corevota/public/docs/minutas_aprobadas/" . $filename; // Ruta para enlaces web

    // Asegurarse de que el directorio exista
    if (!is_dir($saveDir)) {
        if (!mkdir($saveDir, 0775, true)) { // Crear directorio recursivamente con permisos
    ...
    $stmt_doc->execute([
        ':nombre' => $filename,
        ':path' => $pdfWebPath, // Guardar la ruta web
        ':idCreador' => $idPresidenteLogueado // El presidente que aprueba es el creador del doc fijo
    ]);
    ...
    echo json_encode(['status' => 'error', 'message' => 'Ocurrió un error al aprobar la minuta: ' . $e->getMessage()]); // Mostrar error detallado (quitar $e->getMessage() en producción)
} finally {
    // Cerrar conexión explícitamente (buena práctica aunque PDO lo hace)
    $pdo = null;
    $db = null;
}
