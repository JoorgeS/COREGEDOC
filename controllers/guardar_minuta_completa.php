...
class MinutaManager extends BaseConexion
{
  ...
  // Método 1: Obtener nombres de los asistentes
  ...
  // Método 2: Generar el PDF de asistencia
  ...
  // Método 3: Obtener el estado actual de la minuta
  ...
  // Método 4: Enviar el PDF por correo
  ...
  public function guardarMinutaCompleta($idMinuta, $asistenciaIDs, $temasData, $enlaceAdjunto)
  {
    ...
    try {
      ...
      if (!empty($asistenciaIDs)) {
        ...
        // -----------------------------------------------------------------------
        // 🚀 INICIO: ENVÍO DE CORREO ELECTRÓNICO (SOLO SI ES BORRADOR/PENDIENTE) 🚀
        // -----------------------------------------------------------------------
        
        // Condición: Enviar solo si el estado es 'PENDIENTE' (Borrador)
      ...
      if (empty($reunion['fechaTerminoReunion']) && $reunion) { // Solo actualizar si no tiene hora de término
        ...
        error_log("Warning idMinuta {$idMinuta}: No se encontró reunión asociada o ya tenía fechaTerminoReunion.");
      }
            // Corrección: Tu código original no verificaba si ya tenía fechaTermino, pero lo mantendremos simple.
